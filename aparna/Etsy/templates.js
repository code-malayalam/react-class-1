...
const generatePopularContainer = (pItems) => {
  ...
  const arrayOfPopularItems = pItems.map((item) => {
    const popularContents = `
        ...
        <p class="marketPrice">${item.symbol}${numberWithCommas(
      Math.floor(item.actualPrice - item.actualPrice * (item.discount / 100))
    )}</p>
    ...
    function printStars() {
      if (item.star % 1 === 0) {
        ...
        const printedStars =
          FiveStarsForRating.repeat(Math.floor(item.star)) + starsForRating;
        return printedStars;
      }
    }
